...
| 关文件同步时仅同步文件名 | ✅ 已验证 | 2026-06-29，`peer:handled` |
| 对端文件过大拒收 | ✅ 已实现 | `peer:file-too-large:{MB}` → 发送端展示对方回应文案 |
| 512 KB–5 MB 大图分块 | ✅ 已验证 | 2026-06-29 Win 双机 |
| 同步截图 → 资源管理器粘贴为 `.png` | ✅ 已验证 | 2026-06-29 Win 双机 |
...
- 2026-06-29：Win 单文件发送双机验证通过；`peer:*` 回应码与跨设备隐私文案落地
- 2026-06-29：Win 多文件批次（13 个 / 122.5 MB）与接收端资源管理器粘贴验证通过
- 2026-06-29：Win 大图分块、截图粘贴为文件、关文件同步文件名回退验证通过
