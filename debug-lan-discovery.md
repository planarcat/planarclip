...
已确认首个根因是 mDNS 选错本机地址，并已修成优先私有 IPv4。随后结合对端终端日志继续确认到第二个根因：另一台设备把服务注册到了 `172.27.32.1`，这是典型的虚拟网卡 / WSL 网段，不是实际局域网地址。因此本机始终收不到对端在真实局域网里的有效发布。现在已进一步把地址选择规则改成“优先非虚拟网卡的私有 IPv4，再回退到非虚拟网卡的可路由地址”，等待再次做 post-fix 验证。
