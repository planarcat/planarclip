...
- 关闭主窗口后，连接和剪贴板同步在后台仍可进行；主窗 **hide 到托盘** 并保留 WebView，再次打开通常更快。界面要重新打开才能操作，未处理的连接请求靠通知提醒。
- **静默启动**：启动时不创建主窗，仅托盘；首次点托盘会冷启动 WebView（已做首屏优化，不做后台预暖）。未开启静默启动时，启动后会打开主界面。
- 可在设置中把关窗行为改为**直接退出**进程（默认收起到托盘）。
...
- 本文依据 **2026-07-02** 当前产品行为整理（含启动 / 静默 / 关窗 hide、已配对 = 当前连接、熟悉 / 信任分维、首次连接默认信任、移除即断连等）。
